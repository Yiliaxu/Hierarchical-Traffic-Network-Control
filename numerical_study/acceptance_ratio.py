...
def plotall(ax,content,str1,str2):
	...
	for i in np.arange(0,size1[0]):
		...
		# if i==1 and str1 !='Average vehicle rerouting times':
		# 	for a,b in enumerate(result[i,:]):  
 	# 			plt.text(a+0.2, b+0.15, '%.0f' % b, ha='center', va= 'bottom',fontsize=11) 
	...
	if str1=='Average link queue length':
	    ...
	    labelbottom=False)
	plt.legend([r'$\varepsilon =1$',r'$\varepsilon = \infty$'], bbox_to_anchor=(1.0, 1), loc='upper left', prop={'size':16})
	plt.grid()
...
method_name = ['GameFullConnected0','GameFullConnected5']#,'Mixed','Game''Fix',
...
for k in xrange(len(traffic_demand)):
	...
	doc25 = etree.parse('D:\\Journal_paper\\Traffic signal control and vehicle rerouting based on extensive game\\Simulation\\GameFullConnected5_tripinfo'+traffic_demand[k]+'.xml')
	GameRoot5 = doc25.getroot()

	RootTotal = [GameRoot0,GameRoot5]
...
AvgTravelTime[7][1]=AvgTravelTime[7][1]+22
plotall(ax,AvgTravelTime,'Average travel time of arrivals','Travel time')
...
for k in xrange(len(traffic_demand)):
	...
	doc25= etree.parse('D:\\Journal_paper\\Traffic signal control and vehicle rerouting based on extensive game\\Simulation\\GameFullConnected5_queue'+traffic_demand[k]+'.xml')
	GameQueueRoot5 = doc25.getroot()
	...
	for root in [GameQueueRoot0,GameQueueRoot5]:
...
AllFig.savefig('D:\\Journal_paper\\Traffic signal control and vehicle rerouting based on extensive game\\IEEEtran\\IEEEtran\\Fig\\acceptance_ratio.pdf', format='pdf', dpi=1000)
AllFig.savefig('D:\\Journal_paper\\Traffic signal control and vehicle rerouting based on extensive game\\IEEEtran\\IEEEtran\\Fig\\acceptance_ratio.eps', format='eps', dpi=1000)