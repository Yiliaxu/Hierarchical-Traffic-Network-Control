...
def plotall(ax,content,str1,str2):
	...
	if str1=='Average vehicle rerouting times':
	    ...
	    labelbottom=False)
	plt.legend([r'$\lambda=1$',r'$\lambda=10$'], bbox_to_anchor=(1.0, 1), loc='upper left', prop={'size':16})
	plt.grid()
...
for k in xrange(len(traffic_demand)):
	...
	doc22 = etree.parse('D:\\Journal_paper\\Traffic signal control and vehicle rerouting based on extensive game\\Simulation\\Gametest0_tripinfo'+traffic_demand[k]+'.xml')
...
for k in xrange(len(traffic_demand)):
	...
	doc22 = etree.parse('D:\\Journal_paper\\Traffic signal control and vehicle rerouting based on extensive game\\Simulation\\Gametest0_queue'+traffic_demand[k]+'.xml')
