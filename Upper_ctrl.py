...
class Update_policy():
	def __init__(self,T,Tu,ActionRange,N_current,D_current,LoopsOutputNum):
		...
		for i in xrange(3):
			...
			if  MaxOutputBetRegion > self.Max_G[i]:
				...
				self.ActionRange[2*i:2*(i+1),1]=self.ActionRange[2*i:2*(i+1),1]-np.ones(2)*diffV/2.0
		...
		for i in xrange(3):
			self.StateRange[i,0] = N_current[i]-self.ActionRange[2*i,1]-self.ActionRange[2*i+1,1]+(self.Min_P[i]-self.Max_G[i])#*self.UpdateCycle
			self.StateRange[i,1] = N_current[i]+(self.ActionRange[index[i,0],1]+self.ActionRange[index[i,1],1]+self.Max_P[i]-self.Min_G[i])#*self.UpdateCycle
	...
	def STPM_region(self,RegionID,InsideConnection):
		...
		for a in self.ActionSpace.keys():
			...
			for i in xrange(len(state)-1):
				...
				for j in range(3):
					Reward[a][j] += P_s_s_next[j] * (state[j] + state[j + 1]) / 2.0
	...
	def STPM_network(self):
		...
		for i in xrange(len(self.ActionSpace)):
			STPModel_Network[i],Reward_Network[i]=self.CombineStates(STPModel_Region['R1'][i],STPModel_Region['R2'][i],STPModel_Region['R3'][i],Reward_Region['R1'][i],Reward_Region['R2'][i],Reward_Region['R3'][i])
