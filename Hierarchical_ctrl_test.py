    ...
    for i in range(Tsim):
        ...
        if i > warm_up:  ### warm-up the network
            ...
            if i % (Tc * Tu) == 0 and i > 0:
                ...
                upper_controller = Update_policy(Tc, Tu, ActionRange, N_current, D_current, LoopsOutputNum,LinksOccupy)
            ...
            if i % (tc * period) == 0 and i >= Tu * Tc + warm_up:
                ...
                for zone in ['R1', 'R2', 'R3']:
                    ...
                    OptimizeProblem.variables.add(names=["y" + str(j) for j in range(ylen * tc)], obj=y_obj,
                                                  lb=np.zeros(ylen * tc),
